    ...
    <div className="flex h-screen">
      ...
      <div className="flex flex-1 flex-col">
                  ...
                  <CardContent className="p-4">
                    <div className="mb-2 flex items-center justify-between">
                      <Badge variant={message.role === "user" ? "outline" : "default"}>
                        {message.role === "user" ? "You" : availableModels.find(m => m.id === message.modelId)?.name || "AI"}
                      </Badge>
                    ...
                    {message.role === "agent" && (
                      <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Rating 
                            value={ratings[message.id]?.rating || 0}
                            onChange={(value) => message.id && handleRateMessage(message.id, value)}
                            size="sm"
                          />
                        </div>
                      </div>
        ...
        <div className="border-t bg-background p-4">
          ...
          <div className="mx-auto mt-2 max-w-4xl text-xs text-muted-foreground">
            <p>
              Using <strong>{availableModels.find(m => m.id === currentModel)?.name || currentModel}</strong> - {getSystemPromptDescription(currentModel)}
            </p>
          </div>
        </div>
      </div>
